...
pkgdatadir = $(datadir)/playervsplayercoincore
pkgincludedir = $(includedir)/playervsplayercoincore
pkglibdir = $(libdir)/playervsplayercoincore
pkglibexecdir = $(libexecdir)/playervsplayercoincore
...
CONFIG_HEADER =  \
	$(top_builddir)/src/config/playervsplayercoin-config.h
CONFIG_CLEAN_FILES = libplayervsplayercoinconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libplayervsplayercoinconsensus.pc.in \
	...
	$(top_srcdir)/src/config/playervsplayercoin-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/PlayerVsPlayerCoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/PlayerVsPlayerCoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/PlayerVsPlayerCoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/PlayerVsPlayerCoin/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = playervsplayercoin-cli
BITCOIN_DAEMON_NAME = playervsplayercoind
BITCOIN_GUI_NAME = playervsplayercoin-qt
BITCOIN_TX_NAME = playervsplayercoin-tx
...
COPYRIGHT_HOLDERS_FINAL = The PlayerVsPlayerCoin Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = PlayerVsPlayerCoin Core
COPYRIGHT_YEAR = 2018
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/PlayerVsPlayerCoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = playervsplayercoincore
PACKAGE_BUGREPORT = https://github.com/playervsplayercoinpay/playervsplayercoin/issues
PACKAGE_NAME = PlayerVsPlayerCoin Core
PACKAGE_STRING = PlayerVsPlayerCoin Core 0.12.3
PACKAGE_TARNAME = playervsplayercoincore
PACKAGE_URL = https://playervsplayercoin.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/PlayerVsPlayerCoin
abs_srcdir = /tmp/PlayerVsPlayerCoin
abs_top_builddir = /tmp/PlayerVsPlayerCoin
abs_top_srcdir = /tmp/PlayerVsPlayerCoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/PlayerVsPlayerCoin/build-aux/install-sh
...
pkgconfig_DATA = libplayervsplayercoinconsensus.pc
...
empty := 
space := $(empty) $(empty)
OSX_APP = PlayerVsPlayerCoin-Qt.app
...
DIST_CONTRIB = $(top_srcdir)/contrib/playervsplayercoin-cli.bash-completion \
	       $(top_srcdir)/contrib/playervsplayercoin-tx.bash-completion \
	       $(top_srcdir)/contrib/playervsplayercoind.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_playervsplayercoin_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_playervsplayercoin_coverage.info test_playervsplayercoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/PlayerVsPlayerCoin-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/playervsplayercoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/playervsplayercoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/playervsplayercoin-config.h
$(top_srcdir)/src/config/playervsplayercoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/playervsplayercoin-config.h src/config/stamp-h1
libplayervsplayercoinconsensus.pc: $(top_builddir)/config.status $(srcdir)/libplayervsplayercoinconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/PlayerVsPlayerCoin-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/PlayerVsPlayerCoin-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/PlayerVsPlayerCoin-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_playervsplayercoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_playervsplayercoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_playervsplayercoin_filtered.info: test_playervsplayercoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_playervsplayercoin_filtered.info
...
#test_playervsplayercoin_coverage.info: baseline_filtered_combined.info test_playervsplayercoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_playervsplayercoin_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_playervsplayercoin_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_playervsplayercoin_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_playervsplayercoin.coverage/.dirstamp:  test_playervsplayercoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_playervsplayercoin.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_playervsplayercoin.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
