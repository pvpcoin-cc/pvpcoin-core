...
//PlayerVsPlayerCoin only features
...
const char * const BITCOIN_CONF_FILENAME = "playervsplayercoin.conf";
const char * const BITCOIN_PID_FILENAME = "playervsplayercoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "playervsplayercoin" is a composite category enabling all PlayerVsPlayerCoin-related debug output
                if(ptrCategory->count(std::string("playervsplayercoin"))) {
...
#else
    const char* pszModule = "playervsplayercoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PlayerVsPlayerCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PlayerVsPlayerCoin
    // Mac: ~/Library/Application Support/PlayerVsPlayerCoin
    // Unix: ~/.playervsplayercoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PlayerVsPlayerCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PlayerVsPlayerCoin";
#else
    // Unix
    return pathRet / ".playervsplayercoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty playervsplayercoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override playervsplayercoin.conf
